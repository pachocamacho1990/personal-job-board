...
interface FileControllerConfig {
    ownerTable: string;
    filesTable: string;
    foreignKey: string;
    entityLabel: string;
 ...
 */
export function createFileController({ ownerTable, filesTable, foreignKey, entityLabel }: FileControllerConfig) {

    const getFiles = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        try {
            const { id } = req.params;
    ...
    const uploadFile = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        try {
            const { id } = req.params;
    ...
    const deleteFile = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        try {
            ...
            res.json({ message: 'File deleted successfully', id: parseInt(fileId as string, 10) });
        } catch (error) {
            next(error);
        }
    };

    const downloadFile = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        try {
            const { id: ownerId, fileId } = req.params;
